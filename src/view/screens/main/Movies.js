...
const Movies = ({ navigation, route }) => {
  ...
  const getData = () => {
    if (movies) {
      const newData = movies.slice(offset, offset + 2);
      if (!renderData) {
    ...
    if (!movies) {
      ...
      if (!renderData) {
        ...
        const renderItem = ({ item, index }) => {
          if (index === 0 && allMovies[0].is_favorite) {
            return (
              <>
                {allMovies[0].is_favorite ? (
                  <MainImage show={allMovies[1]} navigation={navigation} />
                ) : (
                  <MainImage show={allMovies[0]} navigation={navigation} />
                )}
                <LatestShows
                  navigation={navigation}
          ...
          } else if (index === 0 && !allMovies[0].is_favorite) {
            return (
              <>
                {allMovies[0].is_favorite ? (
                  <MainImage show={allMovies[1]} navigation={navigation} />
            ...
            return (
              <>
                ...
                />
                <LatestWatch
                  title={"Latest Movies Watched"}
                  shows={allMovies.slice(1, 15)}
                  navigation={navigation}
                />
              </>
            );
